<template>
  <div id="navbar2">
        ...
        <TabPane :label="this.Income[0].typeName" name="0">
          <div class="chargePage" v-show="show">
            ...
            <Form :label-width="179" :model="formValidate" :rules="ruleValidate">
              ...
              <FormItem>
                <Row>
                  <Button @click="recharge_in('formValidate')" size="default" type="primary"><a v-if="showA" style="color:#fff" :href="href" target="_blank">下一步</a><span v-else>下一步</span></Button>
                </Row>
              </FormItem>
            </Form>
        ...
        <TabPane :label="this.Income[2].typeName" name="2">
          ...
          <div v-show="!threeBankShow">
            ...
            <div v-show="!redirection">
              ...
              <p style="margin-bottom: 12px;text-align: center">充值金额：{{realAmount}}元</p>
        ...
        <FormItem  label="提现账号：" :required="true">
          <Row>
            ...
            <Select v-model="selectBank">
              <Option v-for="(item,index) in DrawCondition.bankList" :value="item[0]" :key="index">{{item[1]}}</Option>
            </Select>
            </Col>
          </Row>
        </FormItem>
    ...
    <!-- <img src="../../static/img/m5_logo.png" />   -->
        <!-- fc版logo -->
    <!-- <img src="../../static/img/fc_logo.png" />   -->
    <img src="../../static/img/fucai.png" />  
    </div>
    ...
    <div :class="userDetail.type=='0'?'nav_right member':'nav_right tourist'">
      ...
      <div style="width:51px;height:100%;color: #fff;font-size: 14px;line-height: 64px;margin-left: 10px;">
        ...
        <Poptip trigger="hover" placement="bottom">
          ...
          <div  slot="title" style="color: rgba(0,0,0,.65);display: flex;align-items: center;"><Button v-if="userDetail.type=='0'" type="ghost" size="small" @click="amountConversion"><Icon type="loop"></Icon><span style="margin-left: 4px;" >额度转换</span></Button>
          	<div style="width:10px"></div>
          	<Button   type="ghost" size="small" @click="getAllAmount"><Icon type="refresh"></Icon><span style="margin-left:4px;" >刷新</span></Button>
          </div>
          <div slot="content" style="color: rgba(0,0,0,.65)">
          	<div><span>中心钱包：</span><span>{{userDetail.accountBalance.toFixed(2)}}元</span></div>
          	<div v-if="userDetail.type=='0'" style="height: 1px;background:#dddee1;margin: 2px 0;"></div>
          	<div v-if="userDetail.type=='0'"><span>沙巴钱包：</span><span>{{balance_sb}}元</span></div>
          	<div v-if="userDetail.type=='0'"><span>利记钱包：</span><span>{{balance_lj}}元</span></div>
          	<div v-if="userDetail.type=='0'"><span>VG钱包：</span><span>{{balance_vg}}元</span></div>
          </div>
        </Poptip>
      </div>
...
export default {
  ...
  mounted() {
    ...
    this.$watch('formValidate',(newValue)=>{
      if(newValue.name!==''){
        this.showA=true;
      }else{
        this.showA=false;
  ...
  methods: {
    ...
    banks(index) {
      ...
      this.href=this.Bank[index].bankEntrance;
    },
    //选择三方充值方式
    ...
    recharge_in() {
      ...
      this.bankEntrance = this.bankType.bankEntrance;
          ...
          if (data.data.code === 0) {
            ...
            that
              ...
              .then(function(data) {
                ...
                if (mark === "0") {
                  if (
                    data.data.data.respCode === "SUCCESS" &&
                    data.data.data.qrCode !== ""
                  ) {
                    that.realAmount= data.data.data.orderAmount;
        ...
        .then(function(data) {
          ...
          if (name === "0") {
            if (data.data.code === 1) {
              ...
              that.href=that.Bank[0].bankEntrance;
            ...
            if (data.data.code === 2) {
              ...
              that.threeBank[0].gatherMin === 0
                ? (that.value2 = 1)
                : (that.value2 = Math.ceil(that.threeBank[0].gatherMin));
    ...
    confirm() {
      ...
      if (this.notice === true) {
        this.$Modal.confirm({
          title: "",
          content:
            "<h3>是否完成充值？</h3><p>如果您已经完成充值，请点击 已完成</p>",
          okText: "已完成",
          cancelText: "未完成",
          onOk: function() {
            that.cancelOrder("1");
    ...
    withdrawSubmit(name) {
      ...
      that.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (
            that.withdrawMoney + that.DrawCondition.chargesPoint >
            Number(that.DrawCondition.accountBalance)
          ) {
            that.$Notice.warning({
              title: "福彩官方通知",
            ...
            that
              .$http({
                ...
                data: {
                  ...
                  memberConsumeNeed: String(
                    that.DrawCondition.memberConsumeNeed
                  ),
