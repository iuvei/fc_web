		...
		created() {
			...
			if(this.$router.history.current.name != 'register') {
				...
				if(this.userDetail.type=='0'){
					this.register({platform:"gm_ibc"}).then((res) => {
					if(res.code == '0'){
						this.getBalance_sb({platform:"gm_ibc"})
...
//				    });
				...
				this.register({platform:"vg_vg"}).then((res) => {
					if(res.code == '0'){
						this.getBalance_vg({platform:"vg_vg"})
...
//				    });
		...
		methods: {
			...
			websoket() {
				...
				var websocket = new ReconnectingWebSocket(encodeURI(str + URL_1 + "/notify/" + this.userDetail.account));
...
//				    });
				...
				new Promise(function(reslove, reject) {
					if(from.path == '/'&&to.path.indexOf("help")==-1&&to.path!='/register') {
						reslove();
					}
				}).then(() => {
					bus.$emit('announce');
