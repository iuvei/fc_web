<template>
  <div class="members">
    ...
    <!-- 当前用户详情 -->
     <Modal class="mode4" v-model="moda4" width="500" class-name="model_a" :transfer='false'>
      <p slot="header" style="color:#000000;text-align:center">
        <span>当前账号详情</span>
      </p>
      <div style="text-align:center">
        ...
        <div class="mode4_div">
          ...
          <Input v-model="valueQQ" size="large" :placeholder='userDetail.qq' style="width: 300px"/>
        </div>
        <div class="mode4_div">
          <span class="mode4_span">微信:</span>
          <Input v-model="valueWechat" size="large" :placeholder="userDetail.vx" style="width: 300px" />
        </div>
        <div class="mode4_div">
          <span class="mode4_span">电话:</span>
          <Input v-model="valueTel" size="large" :placeholder="userDetail.phone" style="width: 300px" />
        </div>
      </div>
      <div slot="footer">
...
export default {
  data() {
    ...
    return {
      ...
      ruleCustom: {
        ...
        age: [{ validator: validateAge, trigger: "blur", required: true }],
        gender: [
  ...
  created() {
    this.search();
    console.log(this.userDetail, "3213");
  },
  methods: {
    ...mapActions("account", [
    ...
    MemberInfoEdit(accName) {
       if(this.userDetail.qq == null || this.userDetail.qq == ''){
        ...
        }else if(this.userDetail.vx == null || this.userDetail.vx == ''){
          this.userDetail.qq = '您还没有填写微信资料信息'
        }else if(this.userDetail.phone == null || this.userDetail.phone == ''){
          this.userDetail.qq = '您还没有绑定号码'
        }
      if (accName === this.Account) {
       
        this.moda4 = true;
        if (this.userDetail.hierarchy == "1") {
          this.hierarchy = "代理";
        } else {
          this.hierarchy = "会员";
        }
