...
//			},3000)
		...
		methods: {
			...
			updateAwardPeriod() {
				...
				if(this.winList[0].awardNum == null) {
					if(this.kindId == 'fc3d' || this.kindId == 'pl3'||this.kindId == "hk6") {
						this.alreadyOther = 0;
						this.unopened = 1;
						return false
					}
				if(kindIdAry.indexOf(this.kindId)!=-1){
                    ...
                    let nowPeriod = (nextTime.hour() * 60 + nextTime.minute() - 542) / 5 - 1;
					...
					let nextTime = moment(this.winsDragonData.data.nextTime);
                    let nowPeriod = (nextTime.hour() * 60 + nextTime.minute()-4) / 5 ;
				    let restPeriod = periodNoAll - nowPeriod;
					this.alreadyOther = nowPeriod;
					this.unopened = restPeriod
					return false
					...
					let nextTime = moment(this.winsDragonData.data.nextTime);
                    let nowPeriod = (nextTime.hour() * 60 * 60 + nextTime.minute()*60+nextTime.second()-32) / 75;
				    let restPeriod = periodNoAll - nowPeriod;
					this.alreadyOther = nowPeriod;
					this.unopened = restPeriod
					return false
					...
					let nextTime = moment(this.winsDragonData.data.nextTime);
                    let nowPeriod = (nextTime.hour() * 60 * 60 + nextTime.minute()*60+nextTime.second()-14405-40) / 55;
				    let restPeriod = periodNoAll - nowPeriod;
					this.alreadyOther = nowPeriod;
					this.unopened = restPeriod
					return false
				  ...
				  let restPeriod = periodNoAll - nowPeriod
					...
					let nextTime = moment(this.winsDragonData.data.nextTime);
                    let nowPeriod = (nextTime.hour() * 60 + nextTime.minute()) / 1.25 - 1 ;
				    let restPeriod = periodNoAll - nowPeriod;
					this.alreadyOther = nowPeriod;
					this.unopened = restPeriod
					return false
		...
		computed: {
			...mapState(["awarPeriod", "prevAwarPeriod", "classId", "kindId", "winList", "kindList", "gamePlayList", "subGameId", "message", "timeTxts","seconds","userDetail","selectMode","betDataList","winsDragonData"]),
