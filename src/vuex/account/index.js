    ...
    mutations: {
      ...
      conOrCanelWages(state, data){
       ...
       updateOwnInfo(state, data){
        state.updateOwnInfoList = data.data
      },
       // 注册前查询
      queryRegister(state, data){
        state.queryRegisterList = data.data
      }
    },
    actions: {
    	//查询登入记录
      ...
      async registerMember({commit},params){
       ...
       // 注册前查询
       async queryRegister({commit}){
        let res = await fetch('get','queryRegister');
        // commit("queryRegister", res.data)
        return res
      },
